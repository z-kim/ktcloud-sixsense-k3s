...
AWS 위에 K3s 환경을 만들고, 그 위에 ingress-nginx, Argo CD, Checkins 앱(임시), Falco, Node Exporter를 올리고, AWS RDB(MySQL)를 연결합니다.
...
현재 상태를 한 번에 가볍게 확인하고 싶다면 아래 스크립트를 사용할 수 있습니다.

```bash
bash ops-scripts/quick-check-k3s.sh
```

현재 구조에서는 Terraform / Ansible 로 인프라와 K3s bootstrap 까지 준비한 뒤, control-plane 에서 아래 명령을 한 번 실행해야 웹앱 workload 가 실제로 올라갑니다. 현재는 수동 `kubectl apply -k` 확인을 위해 Ansible 이 control-plane 홈 디렉터리에 Checkins workload 경로만 임시로 복사해 둡니다.
...
control-plane은 private subnet 에 단일 EC2로 두었습니다. 이 인스턴스는 Ansible 로 K3s server 를 설치하고, `cluster/bootstrap/` 아래 리소스를 `/var/lib/rancher/k3s/server/manifests/` 에 복사합니다. bootstrap 을 통해 ingress-nginx, Argo CD, Falco, node-exporter 등이 만들어집니다.
...
다만 현재 구조에서는 `checkins-secret` 값만 바뀐 경우 기존 Pod 의 환경변수는 자동으로 갱신되지 않으므로, 필요하면 아래처럼 한 번 다시 rollout 해 주어야 합니다.
...
- Argo CD 설치 방식과 현재 구조에서 어디까지 Argo CD 로 맡기면 좋은지는 [docs/argocd-guide.md](docs/argocd-guide.md) 에 정리해 두었습니다.
